...
float elbow_bend_radians(
    const FlatWorldPose& world,
    size_t upper_arm,
    size_t forearm,
    size_t hand) {
    const vec3 elbow_to_upper =
        normalize(world.positions[upper_arm] - world.positions[forearm]);
    const vec3 elbow_to_hand =
        normalize(world.positions[hand] - world.positions[forearm]);
    return std::acos(std::clamp(
        dot(elbow_to_upper, elbow_to_hand), -1.0F, 1.0F));
}

vec3 arm_pole_direction(
    const FlatWorldPose& world,
    size_t upper_arm,
    size_t forearm,
    size_t hand) {
    return projected_direction(
        world.positions[forearm] - world.positions[upper_arm],
        world.positions[hand] - world.positions[upper_arm]);
}
