...
def _expected_database_bytes(artifacts: ArtifactSet) -> bytes:
    ...
    payload.extend(
        struct.pack("<II", artifacts.contacts.shape[0], artifacts.contacts.shape[1])
    )
...
class DatabaseBuilderTests(unittest.TestCase):
    ...
    def test_lmm_terrain_partition_slices_and_derives_each_source_before_join(self):
        ...
        self.assertTrue(np.all(features.scale[27:31] < np.finfo(np.float32).max))
    ...
    def test_lmm_terrain_features_clamp_future_indices_at_row_256(self):
        ...
        denormalized = features.values[255] * features.scale + features.offset
    ...
    def test_continuity_split_uses_native_local_union_and_drops_short_fragments(self):
        ...
        plan = split_continuity_ranges(native, local, threshold=0.25, minimum_frames=61)
    ...
    def test_matching_features_are_normalized_31d_with_60hz_horizons(self):
        ...
        result = build_matching_features(artifacts, fps=60.0, horizons=(20, 40, 60))
    ...
    def test_clips_become_nonoverlapping_ranges(self):
        ...
        artifacts = combine_clips(
            [HoldenClip.empty(3, 2), HoldenClip.empty(5, 2)],
            skeleton,
        )
    ...
    def test_source_support_samples_root_and_named_toes_in_column_order(self):
        ...
        support = sample_terrain_support(positions, StepTerrain(0.5, 0.29), 0, 2, 3)
    ...
    def test_flat_source_support_is_exact_zero(self):
        ...
        support = sample_terrain_support(positions, FlatTerrain(), 0, 1, 2)
        self.assertEqual(support.dtype, np.dtype(np.float32))
    ...
    def test_support_rejects_bad_indices_and_nonfinite_heights(self):
        ...
        for indices in invalid_indices:
            with self.subTest(indices=indices):
                with self.assertRaisesRegex(ValueError, "support bone indices"):
                    sample_terrain_support(positions, FlatTerrain(), *indices)
        ...
        with self.assertRaisesRegex(ValueError, "finite"):
            sample_terrain_support(positions, Float32OverflowTerrain(), 0, 1, 2)
    ...
    def test_derivatives_are_physical_and_isolated_per_clip(self):
        ...
        shifted[:, 0, 0] += 100.0
        shifted_velocity, shifted_angular = derive_velocities(shifted, rotations, fps)
    ...
    def test_angular_velocity_uses_world_space_increment_order(self):
        ...
        _, angular = derive_velocities(np.zeros((5, 1, 3), np.float64), rotations, fps)

        expected = np.tile([0.0, increment_angle * fps, 0.0], (5, 1))
    ...
    def test_derivatives_reject_bad_shapes_fps_and_quaternions(self):
        ...
        cases = (
            ...
            (np.zeros((3, 1, 3)), np.zeros((3, 1, 4)), 25.0, "quaternion"),
        )
        ...
        with self.assertRaisesRegex(ValueError, "fps"):
            derive_velocities(np.zeros((3, 1, 3)), rotations, "twenty-five")
    ...
    def test_contacts_use_terrain_relative_height_and_speed(self):
        ...
        np.testing.assert_array_equal(penetrated[:, 0], np.zeros(5, np.uint8))
    ...
    def test_contacts_reject_nonfinite_terrain_height(self):
        ...
        with self.assertRaisesRegex(ValueError, "terrain.*finite"):
            derive_contacts(np.zeros((5, 2, 3)), BadTerrain(), 0, 1, 25.0)
    ...
    def test_contact_median_filter_fills_a_one_frame_gap(self):
        ...
        contacts = derive_contacts(positions, FlatTerrain(), 0, 1, 25.0, config)
    ...
    def test_contact_filtering_is_isolated_at_clip_boundaries(self):
        ...
        np.testing.assert_array_equal(incorrectly_joined[:, 0], [1, 1, 1, 0, 0, 0])
    ...
    def test_forward_kinematics_composes_a_noncommuting_three_bone_chain(self):
        ...
        expected_positions[1] = expected_positions[0] + expected_rotation_objects[
            0
        ].apply(local_positions[0, 1])
        expected_positions[2] = expected_positions[1] + expected_rotation_objects[
            1
        ].apply(local_positions[0, 2])

        np.testing.assert_allclose(global_positions[0], expected_positions, atol=1e-7)
        orientation_alignment = np.abs(
            np.sum(global_rotations[0] * expected_rotations, axis=-1)
        )
    ...
    def test_forward_kinematics_rejects_malformed_hierarchies(self):
        ...
        with self.assertRaisesRegex(ValueError, "quaternion"):
            forward_kinematics_arrays(positions, rotations, np.array([-1, 0], np.int32))
    ...
    def test_combine_clips_rejects_invalid_public_inputs(self):
        ...
        with self.assertRaisesRegex(ValueError, "parents"):
            combine_clips(
                [HoldenClip.empty(3, 2)],
                SkeletonSpec(("Simulation", "Hips"), np.array([-1, -1], np.int32)),
            )
    ...
    def test_holden_binary_is_exact_little_endian_and_round_trips(self):
        ...
        artifacts.positions[:] = (
            np.arange(artifacts.positions.size, dtype=np.float32).reshape(
                artifacts.positions.shape
            )
            / 10.0
        )
        artifacts.velocities[:] = -artifacts.positions
        artifacts.rotations[:] = (
            np.arange(artifacts.rotations.size, dtype=np.float32).reshape(
                artifacts.rotations.shape
            )
            / 7.0
        )
        artifacts.angular_velocities[:] = (
            np.arange(artifacts.angular_velocities.size, dtype=np.float32).reshape(
                artifacts.angular_velocities.shape
            )
            / -11.0
        )
        artifacts.contacts[:, 0] = [0, 1, 0, 1]
        artifacts.terrain_support[:] = np.arange(
            artifacts.terrain_support.size, dtype=np.float32
        ).reshape(artifacts.terrain_support.shape)
    ...
    def test_holden_writer_rejects_misaligned_arrays(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            with self.assertRaisesRegex(ValueError, "velocities"):
                write_holden_database(os.path.join(td, "database.bin"), artifacts)
    ...
    def test_holden_reader_rejects_truncation_trailing_and_bad_shapes(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            short_positions = (
                struct.pack("<II", 3, 1)
                + np.ascontiguousarray(artifacts.positions[:3], dtype="<f4").tobytes()
                + payload[8 + position_bytes :]
            )
            corruptions = (
                ("truncated.bin", payload[:-1], "truncated"),
                ("trailing.bin", payload + b"x", "trailing"),
                ("bad-shape.bin", short_positions, "positions|frames|shape"),
            )
