...
def _reach_from_local(
    *,
    ...
    reach = CanonicalReach(
        ...
        endpoint_position_root=world_positions[-1, wrist].astype(np.float32),
        endpoint_rotation_root_wxyz=holden_quat.normalize(
            world_rotations[-1, wrist]
        ).astype(np.float32),
...
def build_captured_reach(
    ...
    stop = source_start + annotation.grab_frame + 1
    if int(corpus.source_frames[stop - 1]) != annotation.source_grab_frame:
    ...
    return _reach_from_local(
        ...
        source_frames=corpus.source_frames[first:stop],
    )
