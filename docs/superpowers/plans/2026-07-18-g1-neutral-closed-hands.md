...
The mapping digest is SHA-256 over canonical ASCII JSON containing schema `mm-sonic-actuator-joint-order/v1` and the ordered names. `verify_loaded_actuator_routing` requires one free root at joint 0, `model.nu == model.njnt - 1`, and for every zero-based actuator slot `int(model.actuator_trnid[slot, 0]) == slot + 1`; its error names the first actuator, actual joint, and expected joint.
