...
**Architecture:** `FunnelCaptureConfig` owns a learned-only 3.0-metre object-radius gate while preserving its 0.45-to-1.00-metre handoff annulus and existing collision-checked connector. A Python evaluation tool reuses the production sampler, projection, and certification functions, varying only the six entry-condition values and writing deterministic JSON evidence.
...
- Produces: a default `FunnelCaptureConfig` whose activation object radius is 3.0 metres while `minimum_object_radius_m` and `maximum_object_radius_m` remain 0.45 and 1.00.
