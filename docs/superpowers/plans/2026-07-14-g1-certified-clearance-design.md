...
Candidate kinds are fixed: `0=face-interior`, `1=edge`, `2=fallback-node`, `3=point`. For kind 1, `candidate_subindex=3*edge_index+local`, where local `0=stationary`, `1=lower endpoint`, and `2=upper endpoint`. For kind 2 it is the immutable uint32 subdivision-node creation ordinal assigned in fixed left-child then right-child order; for kind 0 or 3 it is zero. These numeric values are part of parity output and must not depend on pointer order.
...
Clamp `s_star` to the feasible interval and evaluate it. Also interval-evaluate both feasible endpoints so uncertain clamp classification cannot omit the minimum. The clipped line is empty only when the outward lower enclosure of `dot(q,q)` is strictly greater than the outward upper enclosure of `r*r`; equality is disk tangency and remains feasible with `R=0`. If the radicand or clamp classification is uncertain, retain the unconstrained line minimum as a relaxed patch lower bound and route the patch through fallback. When the projected edge length is zero, XZ is constant and the smaller endpoint Y identifies the exact edge argmin, but the objective at that endpoint remains `min(y0,y1)-sqrt(r*r-|u|^2)`; omitting the sphere term is forbidden.

Every determinant, disk intersection, barycentric sign, and clamp comparison is three-way: certified inside, certified outside, or uncertain. Exact zero/tangency is retained as inside. An uncertain face-membership comparison retains the relaxed `c-r*n` lower bound and invokes fallback. Lower-bound evaluation uses the outward disk/edge intersection and evaluates the stationary point plus both clipped endpoints; an upper witness uses only an inward-certified scalar parameter and the full Section 3 disk/weight feasibility check. No geometric epsilon converts an uncertain sign into inside or outside. A denominator or radicand interval straddling its invalid boundary routes to the bounded fallback.
...
where `rho_min=max(0,rho_interval.lower)` and `rho_interval` certifies the minimum horizontal distance from the origin to the projected closed triangle. Square `rho_min` with a downward enclosure before subtracting it from the upward enclosure of `r*r`, then use the upper enclosure of the square root. Use the lower enclosure of vertex Y. Skip the patch only when the distance interval's lower endpoint is strictly greater than the upper enclosure of `r`; retain exact tangency. The bound is safe because Y is affine over the patch and the square-root term is maximized at the minimum horizontal radius, even when those extrema occur at different points.

Each fallback node carries all three source records as exact root-patch barycentrics, not three independently rounded binary64 weights. The implementation uses reduced nonnegative dyadics `(uint64 numerator, uint16 exponent)` for every root weight. Canonical zero is exactly `(0,0)`; every nonzero value has an odd numerator. A midpoint first aligns denominators with checked shifts, performs a checked integer addition, increments the denominator exponent, and reduces common powers of two. Numerators must remain exactly convertible to binary64 (`<=2^53`) for interval reconstruction. Conversion uses checked `ldexp`: a nonzero numerator that underflows to zero, overflows to nonfinite, or does not round-trip to the identical reduced numerator/exponent fails closed. If a required shift, addition, exponent increment, exact conversion, or round-trip cannot be represented, the call returns transactional `Uncertified` before creating either child; a rounded midpoint is never admitted. Before committing both children, exact dyadic comparison must prove that their shared split vertex is identical, each of their three root-weight sums is exactly one, and their inherited unsplit vertices are bit-identical to the parent records. This representation makes both children tile the parent exactly. Its Y bounds are rebuilt from endpoint expansions at every split.

Attempt a feasible upper witness at every node using the interval-certified closest projected point; reject that attempt unless the full inward disk inequality and normalized-weight checks succeed. When the coarse lower and best feasible upper differ by more than `1e-6 m`, choose the longest edge by tight outward enclosures of squared 3D difference-space length, breaking certified overlap/ties by edge index `0,1,2`. The implementation must prove that the exact source midpoint differs from both parents and that the selected-edge enclosure decreases in the children; otherwise it rotates fairly among overlapping longest edges and ultimately fails closed on no progress. Every created node receives one checked immutable uint32 creation ordinal in fixed left-child then right-child order. That ordinal is both the unique stable queue tie-break and candidate kind 2's `candidate_subindex`; no path bitstring is stored or truncated. Push children in `(lower_bound, creation_ordinal)` order. Stop with `G1ClearanceUncertified` before consuming subdivision node `8193`. Rank-zero and rank-one XZ projections are therefore handled without dividing by a near-zero plane determinant.
