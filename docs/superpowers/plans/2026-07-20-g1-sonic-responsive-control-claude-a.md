# G1 SONIC Responsive Motion-Matching Control — Implementation Plan A (Smallest-Safe-Transport)

**Author:** Claude (plan A of two independent plans for Codex comparison)
**Status:** Implementation-ready plan, report-only milestone `responsive-plan-a`
**Approved contract:** `docs/superpowers/specs/2026-07-20-g1-sonic-responsive-control-design.md`
**Repository revision:** HEAD `7113ebf1673638ef84dc4a783af5fb7e138f2a63` (clean)
...
existing target transport and SONIC consumer can safely accept," with an initial
target of **100 ms** subject to a transport-compatibility probe.
...
- **Stage R1 (transport-preserving).** Reduce irrevocable lookahead to exactly
  ...
  attempt failed.
- **Stage R2 (minimal protocol change), gated behind Probe P0 and P4.** Only if
  P0 shows SONIC's future-reference need is `≤ k` frames with
  `k × 0.02 s < 0.2 s` (i.e. `k ≤ 9`), introduce a **shorter fixed target unit
  of `k` frames** as an explicit, versioned alternative to the 20-frame chunk,
  behind a new schema tag, leaving `target-chunk/v1` untouched as the rollback
  path. If P0 shows `k ≥ 10`, Stage R2 is abandoned and the plan terminates at
  the R1 lower bound with an explicit "sub-200 ms infeasible on this policy"
  report.
...
|-------|--------------------|------|--------------|
| **P0** | How many future reference frames `k` does unmodified `g1_deploy_onnx_ref` require ahead of the presented physics frame (via `catch_up` and merged-tail offset)? | Dynamic (GEAR) | Any sub-200 ms claim; Stage R2 |
...
**P0 is the falsification hinge.** Its RED form asserts a deliberately-wrong
sentinel `k` that the real binary will contradict, forcing the true value to be
measured rather than assumed. If the measured `k × 0.02 s ≥ 0.2 s`, the plan
records sub-200 ms as infeasible for this policy and stops at R1.
...
- [ ] Step 2.4 — Commit as commit boundary 2 ("feat: prove MM-generated direction signs from targets").

### Task 3: Responsive prefix scheduler (Stage R1)

- **New module:** `sonic/python/mm_sonic/responsive_scheduler.py`. A thin
  orchestrator that owns exactly one committed, not-yet-consumed prefix and
  drives the *existing* `Coordinator.run_one_chunk` (`coordinator.py:950`)
  one chunk at a time, latching the latest mailbox command at each boundary and
  emitting a `BoundaryTrace` (Task 1). It sets effective lookahead to **one
  committed chunk** and never pre-commits more than one future chunk.
- **New test file:** `tests/python/test_sonic_responsive_scheduler.py`, using
  ...
  - `test_stale_candidate_from_newer_latched_command_is_aborted_before_publish`
    (**Probe P2**). Latches a new command mid-generation; asserts the superseded
    candidate is aborted via `TargetTimeline.abort` (`timeline.py:571`) and
    `mm.abort` (`process.py`/`coordinator.py:842-848`) and that
    `publisher.send_prepared` is **never** called for it. This drives the real
    coordinator abort branch, not a helper.
...
- **Minimal change:** implement the scheduler as a boundary loop
  (structurally like `operator_runtime.py:run_operator_boundary_loop`) that
  calls `Coordinator.run_one_chunk` with a freshly latched command per boundary,
  records a `BoundaryTrace`, and on any generation/validation failure leaves the
  coordinator in its terminal paused verdict without releasing steps.
- **GREEN command:** same as RED.
- **Commit boundary 3:** "feat: single-prefix responsive scheduler with pause-on-miss."

Checklist:

- [ ] Step 3.1 — Write the four RED tests in `tests/python/test_sonic_responsive_scheduler.py` reusing the `test_sonic_coordinator.py` injected fakes; run RED and confirm `ModuleNotFoundError: mm_sonic.responsive_scheduler`.
- [ ] Step 3.2 — Implement the single-prefix boundary loop over `Coordinator.run_one_chunk`, emitting one `BoundaryTrace` per boundary and latching the newest command.
- [ ] Step 3.3 — Drive the real coordinator abort branch so a superseded candidate calls `TargetTimeline.abort` and `mm.abort` and never reaches `publisher.send_prepared`; assert release ordering strictly after commit+publish.
- [ ] Step 3.4 — Force a raising `mm.generate` and assert terminal `simulation_paused=True` with `gate.release_steps` uncalled; run GREEN and commit as commit boundary 3.
...
- **New test:** `tests/python/test_sonic_stage_b.py` (or a sibling
  ...
  - `test_measures_sonic_future_reference_frame_requirement` (**Probe P0**).
    Drives the pinned `g1_deploy_onnx_ref` through the existing
    `wait_for_stream_processing` transcript path (`process.py:2554`) and the
    `catch_up` field (`zmq_v1.py:281`) to measure the minimum number of appended
    reference frames that must lead the presented physics frame. RED form
    asserts a deliberately-wrong sentinel `k`; the real binary contradicts
    it, forcing measurement.
- **RED/GREEN commands:** controller-owned dynamic-gate invocation (this plan
  does not run GPU/GEAR). Expected RED: sentinel-`k` assertion fails with the
  measured value in the message. GREEN: assertion updated to the measured `k`
  and recorded in the run evidence.
- **Decision rule (recorded in evidence):**
  - If `k ≤ 9` (`k × 0.02 s < 0.2 s`): proceed to Stage R2.
  - If `k ≥ 10`: **stop at R1**, report sub-200 ms infeasible for the unmodified
    policy, and publish the 0.4 s + generation floor as the honest result.
- **Commit boundary 5:** "test: measure SONIC future-reference frame requirement."

Checklist:

- [ ] Step 5.1 — Add `test_measures_sonic_future_reference_frame_requirement` with a deliberately-wrong sentinel `k`, wired through `wait_for_stream_processing` and the `catch_up` field; hand it to the controller for the dynamic-gate RED run.
- [ ] Step 5.2 — Record the measured `k` from the controller-run GREEN, update the assertion to the measured value, and store it in run evidence.
- [ ] Step 5.3 — Apply the decision rule (`k ≤ 9` → Stage R2; `k ≥ 10` → stop at R1 with the infeasibility verdict) and commit as commit boundary 5.

### Task 6: Minimal short-unit protocol variant (Stage R2, only if P0 permits)

- **Change (versioned, additive):** introduce `target-chunk/v2` with a fixed
  `k`-frame width alongside the untouched `target-chunk/v1` (`timeline.py:86`),
  a parallel resample width, and a `(count-1) % k` reference invariant, selected
  only by the responsive path. `v1` remains the rollback format.
- **New tests:** extend `tests/python/test_sonic_timeline.py` and
  `tests/python/test_resample.py` with `k`-width analogues of
  `test_prepare_assigns_one_through_twenty_without_mutating_accepted_state:103`
  and the 11→20 resample contract, plus a
  `test_v2_prefix_presentation_latency_lower_bound_is_k_frames` assertion.
- **RED command:**
  `/home/ubuntu/miniconda3/envs/diffsim/bin/python -m unittest tests.python.test_sonic_timeline tests.python.test_resample -v`
- **Expected RED:** shape/`%20` assertions reject the `k`-width chunk until v2 is
  implemented.
- **GREEN command:** same as RED.
- **Commit boundary 6:** "feat: additive short-unit target-chunk/v2 for sub-200 ms prefix."

Checklist:

- [ ] Step 6.1 — Only if Task 5 yielded `k ≤ 9`: add the `k`-width RED tests to `tests/python/test_sonic_timeline.py` and `tests/python/test_resample.py`; run RED and confirm the shape/`%20` rejections.
- [ ] Step 6.2 — Implement additive `target-chunk/v2` (fixed `k` width, parallel resample width, `(count-1) % k` invariant) selected only by the responsive path, leaving `v1` untouched as rollback.
- [ ] Step 6.3 — Run GREEN including `test_v2_prefix_presentation_latency_lower_bound_is_k_frames`; commit as commit boundary 6.
...
- **gate-001** — the plan-a-quality verifier over this file
  (`docs/superpowers/plans/2026-07-20-g1-sonic-responsive-control-claude-a.md`).
  The controller's pre-change RED proof was `FileNotFoundError` on this exact
  path; creating this file is the corresponding GREEN precondition.
...
- **Flat dynamic gate** — unmodified SONIC stays upright across direction and
  ...
  **separately** (design §"Latency Contract" published fields), and p95
  ...
  solely if Task 5 (P0) has established `k ≤ 9` and Stage R2 is in effect.
...
2. A ≤ 200 ms p95 claim is admissible **only** after Probe P0 establishes that
   SONIC's future-reference need is `k ≤ 9` frames and Stage R2's `k`-frame unit
   is in effect. If P0 yields `k ≥ 10`, the deliverable is the R1 floor plus a
   recorded "sub-200 ms infeasible on unmodified policy" verdict.
...
- **Risk A — sub-200 ms is infeasible on the unmodified policy.** If Probe P0
  measures `k ≥ 10`, the 20-frame floor cannot shrink safely. *Mitigation:* the
  plan fails closed at the R1 floor and reports the measured minimum atomic
  duration rather than claiming 200 ms; no protocol edit is attempted.
...
- **Observed from code:** the 0.8–1.6 s lookahead arithmetic (§1.1); the fixed
  ...
  frames (§1.4).
- **Inferred / made into probes:** the exact SONIC future-reference frame count
  `k` (Probe P0, lives in the pinned binary); the per-direction generated-root
  magnitudes (Probe P1); the one-chunk stop-displacement outcome (Probe P4).
