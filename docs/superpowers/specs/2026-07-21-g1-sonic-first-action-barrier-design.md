...
After the final physics release, the driver immediately takes GEAR's official
stop path while LowState is fresh, then builds evidence with GEAR already
reaped so post-run serialization cannot trip that watchdog.
