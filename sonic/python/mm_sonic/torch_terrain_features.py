...
def dense_query_points(
    root_xy: torch.Tensor,
    root_yaw: torch.Tensor,
    trajectory: CommandTrajectory,
) -> torch.Tensor:
    """Return 91 body-frame footprint points plus four command-path points."""

    local = _dense_local_points(device=root_xy.device, dtype=root_xy.dtype)
    body_patch = root_xy + _rotate_xy(local, root_yaw)
    root_facing = torch.stack((torch.cos(root_yaw), torch.sin(root_yaw)))
    path_profile = _arc_distance_points(
        root_xy,
        trajectory.position_world_xy,
        trajectory.facing_world_xy,
        root_facing,
    )
    return torch.cat((body_patch, path_profile), dim=0)
...
class TerrainFeatureExtension:
    ...
    def dimension(self) -> int:
        return 4 if self.condition == "legacy" else 95
    ...
    def query_row(
        ...
        if self.condition == "dense":
            matcher_points = dense_query_points(root[:2], yaw, trajectory)
